...
<form id="comtra_search" method="POST" action="<?php echo $this->action('core'); ?>">
	...
	<?php View::element('translate/select_locales', ['officialLocales' => $officialLocales, 'otherLocales' => $otherLocales, 'checkedLocales' => $checkedLocales], 'community_translation'); ?>
	<button type="submit" class="btn btn-primary"><?php echo t('Search'); ?></button>
</form>
<?php
if (isset($packages)) {
            ?><h3><?php echo h('Core versions'); ?></h3><?php
    View::element('translate/package_stats', ['locales' => $checkedLocales, 'packages' => $packages, 'stats' => $stats, 'translatedThreshold' => $translatedThreshold], 'community_translation');
        }
    break;
